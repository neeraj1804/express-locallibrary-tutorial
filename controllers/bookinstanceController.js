...
exports.bookinstance_list = function(req, res, next) {
    BookInstance.find().populate("book").exec(function(err, book_instances){
        if(err){
            next(err);
        }else{
            res.render('bookinstance_list', {title: "Book Instance List", bookinstance_list: book_instances});
        }
    });
};
...
exports.bookinstance_detail = function(req, res, next) {
    var bookinstance_id = req.params.id;
    BookInstance.findById(bookinstance_id).populate("book").exec(function(err, bookinstance){
        if(err){
            next(err);
        }else{
...
exports.bookinstance_create_post = function(req, res) {
    ...
    req.checkBody('due_back', 'Invalid date').optional({ checkFalsy: true }).custom(value => {
